...
| Claim | Value | Source | Commit |
|---|---|---|---|
| Render alone | median of per-chip medians 0.583 px; mean of medians 0.797 px; median points 29.5 (median of `render_n` over all 130 chips, the 7 unmatched chips counted as zero; the arm medians of Table I are over 130 chips with no unmatched chip in any seed); 7 of 130 chips with no match | `evidence/osm_render_baseline/render_baseline_summary.json` | `c263e4c` |
...
| II-F variance share (P11 A.1) | between (2.89/17.3)² = 2.8 % and (2.89/14.5)² = 4.0 %; the record's 3.9 % is underived (entry 51) | `paper-context-addendum.md` §19 inputs; arithmetic inline in the letter | `aa0f82c` |
| II-D residual range (P11 A.2) | 1.4–2.1 px: Table I fine-tuned arm means 1.393–2.070 | six-seed CSVs | `aa0f82c` |
| III-L equal-count movement (P11 A.3) | six-seed mean per arm moves ≤ 0.019 px (C1 0.015, C2 0.019, C4 0.003, C5 0.003); seed-arm comparisons up to 0.082 px (C2), 0.073 (C1), 0.062 (C5), 0.058 (C4); advantages 0.44–1.22 px | `evidence/osm_render_baseline/render_baseline_summary.json`, raw vs equal_count per arm and seed; results amendment 14 Sep | `aa0f82c` |
| IV-C MI censoring (P11 A.4) | 70 of 390 Ankara MI measurements at the ±8 px bound, 17.9 %: pretrained 36/130, C1 23/130, C2 11/130 (pooled over four sets it is 147/930 = 15.8 %) | `evidence/B3/B3_scores.csv`, `mi` ≥ 8 | `aa0f82c` |
