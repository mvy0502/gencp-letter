...
> **Restored 13 September 2026 from `284571b`.** This file was created in `284571b`
> (26 August) and deleted in `6750978` ("docs: refresh the handover snapshot at the plugin
> milestone", 27 August). Nothing referenced the file, so the deletion principle's question
> — *does anything read it?* — returned no, and a snapshot refresh that copies a curated
> tree swept it away with the rest. Seventeen days later a session searching the record for
> a drafted entry 35 concluded it had never existed. That is the failure this entry
> records, one level up: a drafted correction about evidence that went missing was itself
