...
below the grid, and the three seed-level contrasts of Section III-B drawn as arrows from
...
BH, BG = 0.60, 0.28                            # the fifth arm's box, below the grid (P14 A), and its gap
X0, Y0 = 0.98, 0.15 + BH + BG                  # grid origin (room for row headers; the box sits under it)
...
fig, ax = plt.subplots(figsize=(3.5, 2.05)); ax.set_axis_off()
...
PW = 1.70; GRID_CX = X0 + W + GX / 2; PX = GRID_CX - PW / 2; PY = 0.15   # centred under the grid
ax.add_patch(FancyBboxPatch((PX, PY), PW, BH, boxstyle="round,pad=0,rounding_size=0.04", fc="white", ec="black", lw=0.8, ls=(0, (3, 2))))
ax.text(PX + PW / 2, PY + BH / 2 + 0.16, "pretrained", ha="center", va="center", fontsize=6.2)
ax.text(PX + PW / 2, PY + BH / 2 + 0.02, "repository code: pre", ha="center", va="center", fontsize=4.8, color="0.35")
ax.text(PX + PW / 2, PY + BH / 2 - 0.16, "adv. + LPIPS objective;\nEuropean training data", ha="center", va="center", fontsize=4.6)
...
ax.set_xlim(0, 2 * GRID_CX); ax.set_ylim(0, Y0 + 2 * H + GY + 0.44)   # x-limits symmetric about the grid centre
